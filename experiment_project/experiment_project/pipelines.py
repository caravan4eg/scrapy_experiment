...
class QuotesJSONPipeline(object):
    ...
    Store data to CSV is processing automaticly by tuning in settings and
...
class QuotesDBPipeline(object):
    ...
    def process_item(self, item, spider):
        ...
        print('\n**************** QuotesDBPipeline is processing data to DB ****************')
        print(f'{str(item["author"]).upper()}: {str(item["tags"]).upper()}')
        print('************************************************************\n')

        try:
            self.cur.execute('insert into public.quotes_new_table (text, author, tags) values (%s, %s, %s);',
